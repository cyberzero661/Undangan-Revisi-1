...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!supabase || !isConfigured) {
      setError("Konfigurasi Supabase belum lengkap. Pastikan file .env.local sudah benar.");
      return;
    }
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-gray-100">
          ...
          {!isConfigured && (
            <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-xl text-yellow-700 text-sm">
              <strong>Perhatian:</strong> Konfigurasi Supabase belum lengkap. Pastikan file <code className="bg-yellow-100 px-1 rounded">.env.local</code> sudah benar.
            </div>
          )}
